...
<script>
    ...
    /**% left edge of separator*/$: edges = Array(separators.length)
        .fill(100/panes.length).map((n, i) => n*(i+1))
    /**
    ...
    */let curr = null;
    ...
    const onPointermove = (/** @type PointerEvent */ e) => {
        ...
        var deltaX = Math.min(Math.max(e.clientX - curr.e.clientX, -curr.firstWidth), curr.secondWidth);
        //convert deltaX to %
        edges[i] = 100*(curr.offsetLeft + deltaX)/container.offsetWidth;
        sliders[i] = 100 * (curr.firstWidth + deltaX) / (paneElems[i].offsetWidth + paneElems[i+1].offsetWidth)
        ...
        if(i < sliders.length) sliders[i+1] = 100 * widths[i+1] / (widths[i+1] + widths[i+2])
    }
...
<div class="wrapper" bind:this={container}
    ...
    {#each panes as p, i}
        ...
        {#if i !== panes.length - 1}
            <div class="separator" bind:this={separators[i]} 
                style="left: {edges[i]}%"
