...
for(const tag of fontTags) {
    data.fonts.push({
        ...
        fontStyle: tag.querySelector("sd[property=font-style]")!.attributes.getNamedItem("value")!.value,
        fontWeight: tag.querySelector("sd[property=font-weight]")!.attributes.getNamedItem("value")!.value
    });
}
...
// Color themes
const colorThemeTags = document.getElementsByTagName("color-themes")[0].getElementsByTagName("color-theme");
data.themes = [];
...
for(const tag of colorThemeTags) {
    ...
    if(tag.attributes.getNamedItem("default")?.value === "true") data.defaultTheme = data.themes[data.themes.length - 1].name;
}
...
for(const tag of traitTags) {
    data.traits[tag.attributes.getNamedItem("name")!.value] = tag.attributes.getNamedItem("value")?.value;
}
...
for(const tag of booksTags) {
    ...
    for(const feature of featuresTags) {
        features[feature.attributes.getNamedItem("name")!.value] = parseConfigValue(feature.attributes.getNamedItem("value")!.value);
    }
    ...
    for(const book of bookTags) {
        books.push({
            ...
            chapters: parseInt(book.getElementsByTagName("ct")[0].attributes.getNamedItem("c")!.value),
            chaptersN: book.getElementsByTagName("cn")[0].attributes.getNamedItem("value")!.value,
    ...
    data.bookCollections.push({
        ...
        books,
        style: {
            font: stylesTag.getElementsByTagName("text-font")[0].attributes.getNamedItem("family")!.value,
            languageCode: writingSystem.attributes.getNamedItem("code")!.value,
            languageName: writingSystem.getElementsByTagName("display-names")[0].getElementsByTagName("form")[0].innerHTML,
            lineHeight: parseInt(stylesTag.getElementsByTagName("line-height")[0].attributes.getNamedItem("value")!.value),
            numeralSystem: stylesTag.getElementsByTagName("numeral-system")[0].attributes.getNamedItem("value")!.value,
            textDirection: stylesTag.getElementsByTagName("text-direction")[0].attributes.getNamedItem("value")!.value,
            textSize: parseInt(stylesTag.getElementsByTagName("text-size")[0].attributes.getNamedItem("value")!.value),
            verseNumbers: stylesTag.getElementsByTagName("verse-number-style")[0].attributes.getNamedItem("value")!.value
        }
    });
}
console.log(`Converted ${data.bookCollections.length} book collections with [${data.bookCollections.map(x => x.books.length).join(", ")}] books`);


// Menu localizations
data.translationMappings = {};
const translationMappingsTags = document.getElementsByTagName("translation-mappings")[0].getElementsByTagName("tm");
...
for(const tag of translationMappingsTags) {
    ...
    for(const localization of tag.getElementsByTagName("t")) {
        localizations[localization.attributes.getNamedItem("lang")!.value] = localization.innerHTML;
    }
...
if(document.getElementsByTagName("keys").length > 0) {
    data.keys = Array.from(document.getElementsByTagName("keys")[0].getElementsByTagName("key")).map(key => key.innerHTML);
